    ...
    <ThemedView style={{ flex: 1, backgroundColor: '#fff' }}>
      ...
      <View style={styles.header}>
        <TouchableOpacity onPress={() => router.back()} style={styles.backBtn}>
          <IconSymbol name="chevron.right" size={24} color={Colors.light.icon} style={{ transform: [{ rotate: '180deg' }] }} />
        </TouchableOpacity>
      ...
      <View style={styles.tabs}>
        <TouchableOpacity onPress={() => setTab('pending')} style={[styles.tab, tab === 'pending' && styles.tabActive]}>
          <ThemedText style={[styles.tabText, tab === 'pending' && styles.tabTextActive]}>Pending</ThemedText>
        </TouchableOpacity>
        <TouchableOpacity onPress={() => setTab('upcoming')} style={[styles.tab, tab === 'upcoming' && styles.tabActive]}>
          <ThemedText style={[styles.tabText, tab === 'upcoming' && styles.tabTextActive]}>Upcoming</ThemedText>
        </TouchableOpacity>
      </View>
      {/* List */}
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.paymentRow}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <ThemedText type="default" style={styles.due}>{item.due}</ThemedText>
            </View>
            <ThemedText type="defaultSemiBold" style={styles.amount}>{item.amount}</ThemedText>
          </View>
        )}
        style={{ flex: 1 }}
